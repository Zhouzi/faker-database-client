...
describe('Collection', function () {
  ...
  it('should return a random collection if length is not provided', function () {
    var callback = function () {};
    var length1 = Collection(callback).length;
    var length2 = Collection(callback).length;
    var length3 = Collection(callback).length;
    assert.notEqual(length1 + length2 + length3, 3 * 10);
  });
});
